...
def converter():
    ...
    while i >= 0:
        ...
        # (but not quite since theres some textures with transparent parts)
        ...
        rgba = str(round(rgbImgColor.rgb_r * 255)) + ', ' + str(round(rgbImgColor.rgb_g * 255)) + ', ' + str(round(rgbImgColor.rgb_b * 255)) + ', ' + str(imgColor[3])
        xyz = str(xyzImgColor.xyz_x) + ', ' + str(xyzImgColor.xyz_y) + ', ' + str(xyzImgColor.xyz_z)
        print("added " + str(imgName) + "	with rgb value: " + rgba)


        # append result to the block-storing JS object
        outputTxt = open(dirname + '.js', 'a')
        
        # Write to the .json file   (but its actually just a .js)
        if i == 0:       #without a comma at the end
            outputTxt.write('    { id: "' + imgName + '", rgba: [' + rgba + '], xyz: [' + xyz + '] }\n')
        else:            #with a comma,
            outputTxt.write('    { id: "' + imgName + '", rgba: [' + rgba + '], xyz: [' + xyz + '] },\n')
